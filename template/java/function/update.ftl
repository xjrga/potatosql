...
public class ${table.table_name?cap_first}_upd {
    private final Connection connection = Connect.getInstance().getConnection();
    public void apply( ${table.table_name?cap_first} o ) {
        try ( CallableStatement proc = connection.prepareCall( "{CALL ${data.schema_name}.${table.table_name}_update(<#list table.key_iterator as key>?<#if key?has_next>,</#if></#list>)}")) {
