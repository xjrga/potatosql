...
public class ${table.table_name?cap_first}_ins {
  ...
  public ${table.table_name?cap_first}_ins() {
    this.connection = null;
  }

  public void execute(<#list table.key_iterator as key>
    ${dtype.getJava(key.datatype)} ${key.key_name?lower_case}<#if key?has_next>,</#if></#list>
  ) {
    try (CallableStatement proc = connection.prepareCall("{CALL schema.procedure(<#list table.key_iterator as key>?<#if key?has_next>,</#if></#list>)}")) {
