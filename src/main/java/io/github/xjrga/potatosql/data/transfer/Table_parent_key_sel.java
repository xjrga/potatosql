...
public class Table_parent_key_sel implements R1<Relationship, List<Key>> {
  ...
  public List<Key> apply(Relationship relationship) {
    ...
    try (CallableStatement proc =
        connection.prepareCall("{CALL potatosql.Table_parent_key_select(?,?)}"); ) {
      proc.setInt(1, relationship.getSchema_id());
      proc.setInt(2, relationship.getTable_id_parent());
