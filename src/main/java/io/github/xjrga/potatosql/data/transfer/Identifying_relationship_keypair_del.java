...
public class Identifying_relationship_keypair_del
        implements V1<Keypair> {
    private final Connection connection = Connect.getInstance().getConnection();
    @Override
    public void apply( Keypair pair ) {
        try ( CallableStatement proc = connection.prepareCall( "{CALL potatosql.Identifying_relationship_key_pair_delete(?,?,?,?,?,?)}" ) ) {
