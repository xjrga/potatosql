...
public class Relationship_sel implements R1<Schema, List<Relationship_mod>> {
  ...
  public List<Relationship_mod> apply(Schema schema) {
    ...
    BeanListHandler<Relationship_mod> beanListHandler =
        new BeanListHandler<>(Relationship_mod.class);
    try (CallableStatement proc =
        connection.prepareCall("{CALL potatosql.Relationship_select(?)}"); ) {
