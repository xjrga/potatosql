...
public class Template_key_sel implements R2<Schema, Template_table, Template_table> {
  ...
  public Template_table apply(Schema schema, Template_table table_in) {
    ...
    try (CallableStatement proc =
        connection.prepareCall("{CALL potatosql.find_table_keys(?,?)}"); ) {
      ...
      while (rs.next()) {
        ...
        table_out.add_key(
            new Template_key(key_name, is_primary_key, datatype, key_order, is_foreign_key));
      }
