...
public class Relationship_ins_auto
        implements R1<Relationship, Integer> {
    private final Connection connection = Connect.getInstance().getConnection();
    @Override
    public Integer apply( Relationship key ) {
        Integer id = null;
        try ( CallableStatement proc = connection.prepareCall( "{CALL potatosql.Relationship_insert_auto(?,?,?,?,?)}" ) ) {
