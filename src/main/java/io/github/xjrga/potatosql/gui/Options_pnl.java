...
public class Options_pnl
    ...
    private void setup() {
        ...
        collection_dialect = collection_config.stream()
                .map( dcode -> dcode.getDialect() ).distinct()
                .sorted().collect( Collectors.toList() );
        lst_dialect.setModel( populator.apply( collection_dialect ) );
        lst_dialect.addListSelectionListener( ( ListSelectionEvent e )
                -> {
        ...
        lst_category.addListSelectionListener( ( ListSelectionEvent e )
                -> {
            if ( e.getValueIsAdjusting() ) {
                return;
            }
        ...
        lst_action.addListSelectionListener( ( ListSelectionEvent e )
                -> {
            if ( e.getValueIsAdjusting() ) {
                return;
            }
    ...
    private void event_list_selection_dialect() {
        ...
        List<Code> collection_code = collection_config.stream()
                .filter( ( Code code ) -> code.getDialect().equals( selected_dialect ) )
                .sorted( comparator ).collect( Collectors.toList() );
        List<String> collection_category = collection_code.stream()
                .map( code -> code.getCategory() ).distinct()
                .sorted().collect( Collectors.toList() );
        Code_populator populator = new Code_populator();
    ...
    private void event_list_selection_category() {
        ...
        List<Code> collection_code = collection_config.stream()
                .filter( ( Code code ) -> (code.getCategory().equals( selected_category ) & code.getDialect().equals( selected_dialect )) )
                .sorted( comparator_code ).collect( Collectors.toList() );
        Comparator<Code_sub> comparator_code_sub = Comparator.comparing( code_sub -> code_sub.getAction() );
        List<Code_sub> collection_action = collection_code.stream()
                .map( code -> code.getCode_sub() ).distinct()
                .sorted( comparator_code_sub ).collect( Collectors.toList() );
    ...
    private void event_action_add() {
        selected_actions
                .forEach( ( selected_action ) -> model_queue
                .addElement( collection_config.stream()
